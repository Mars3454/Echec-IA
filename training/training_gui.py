...
class TrainingThread(threading.Thread):
    def __init__(self, app, mode):
        ...
        self.app=app; self.mode=mode; self.running=True; self.paused=False

    def run(self):
        if self.mode=="training": self._run_training()
        else: self._run_match()

    def _run_training(self):
        app=self.app
        try:
            gen=app.current_generation; best=app.best_weights.copy()
        ...
        try:
            ...
            for gn in range(1, app.match_games+1):
                ...
                if gn%2==1:
                    ...
                    if r==1: wa+=1; app.update_log(f"  A gagne (Blancs)")
                    elif r==-1: wb+=1; app.update_log(f"  B gagne (Noirs)")
                    else: draws+=1; app.update_log("  Nulle")
                else:
                    r=self._play_game(app.match_weights_b, app.match_weights_a)
                    if r==1: wb+=1; app.update_log(f"  B gagne (Blancs)")
                    elif r==-1: wa+=1; app.update_log(f"  A gagne (Noirs)")
                    else: draws+=1; app.update_log("  Nulle")
...
class TrainingGUI:
    def __init__(self, root):
        ...
        self.search_depth=2; self.max_plies=160; self.mutation_step=3; self.accept_margin=0.02
        self.match_games=20; self.match_weights_a=DEFAULT_WEIGHTS.copy(); self.match_weights_b=DEFAULT_WEIGHTS.copy()
        self.match_weights_a_name="Poids A"; self.match_weights_b_name="Poids B"
        self.training_history=[]
        self._load_current_state(); self._build_interface()

    def _load_current_state(self):
        ensure_defaults(); self.current_generation=latest_generation_index()
        try: self.best_weights=load_gen(gen_path(self.current_generation))
        except: self.best_weights=DEFAULT_WEIGHTS.copy()
    ...
    def _build_training_tab(self, parent):
        ...
        for i,(lbl,attr,val,mn,mx) in enumerate(rows):
            ...
            v=tk.IntVar(value=val); setattr(self,attr,v)
    ...
    def _build_stats_tab(self, parent):
        ...
        sb.config(command=self.stats_text.yview); self.update_stats_display()

    def start_training(self):
        if self.is_training: return
        self.max_iterations=int(self.iterations_var.get()); self.games_per_eval=int(self.games_eval_var.get())
        self.movetime_ms=int(self.movetime_var.get()); self.search_depth=int(self.depth_var.get())
        self.max_plies=int(self.plies_var.get()); self.mutation_step=int(self.mutation_var.get())
    ...
    def start_match(self):
        if self.is_training: return
        self.match_games=int(self.match_games_var.get()); self.is_training=True; self.current_mode="match"
        self.start_match_btn.config(state="disabled"); self.match_log_text.delete(1.0,tk.END)
        self.training_thread=TrainingThread(self,"match"); self.training_thread.start()
    ...
    def update_progress(self, cur, tot):
        def _u():
            self.progress_var.set((cur/tot)*100 if tot>0 else 0); self.progress_label.config(text=f"{cur} / {tot}")
        self.root.after(0,_u)

    def add_training_stats(self, stats): self.training_history.append(stats)
    ...
    def load_weights(self, side):
        fn=filedialog.askopenfilename(title=f"Charger poids {side.upper()}",filetypes=[("JSON","*.json"),("Tous","*.*")])
        if not fn: return
        try:
            with open(fn) as f: data=json.load(f)
            w=data.get("weights",data); name=os.path.basename(fn)
            setattr(self,f"match_weights_{side}",w); setattr(self,f"match_weights_{side}_name",name)
    ...
    def use_best_weights(self, side):
        try:
            w=get_best_weights(); name=f"Best (gen {self.current_generation})"
            setattr(self,f"match_weights_{side}",w); setattr(self,f"match_weights_{side}_name",name)
            getattr(self,f"weights_{side}_label").config(text=name)
        except Exception as e: messagebox.showerror("Erreur",str(e))
    ...
    def use_generation(self, side):
        ...
        def load():
            try:
                w=load_gen(gen_path(gv.get())); name=f"gen_{gv.get()}"
                setattr(self,f"match_weights_{side}",w); setattr(self,f"match_weights_{side}_name",name)
                getattr(self,f"weights_{side}_label").config(text=name); win.destroy()
            except Exception as e: messagebox.showerror("Erreur",str(e))
        ttk.Button(win,text="Charger",command=load).pack(pady=10)


def main():
    root=tk.Tk(); TrainingGUI(root); root.mainloop()
